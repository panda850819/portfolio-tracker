...
import StockAssetsList from "@/components/stock-assets-list"
import CryptoAssetsList from "@/components/crypto-assets-list"
import WalletAssetsList from "@/components/wallet-assets-list"
import DeFiAssetsList from "@/components/defi-assets-list"
import CashAssetsList from "@/components/cash-assets-list"
...
export default function AssetsPage() {
  ...
  const walletAssets = assets.filter(a => a.type === 'wallet')
  ...
  const defiTotal = defiAssets.reduce((sum, asset) => sum + asset.market_value, 0)
  const walletTotal = walletAssets.reduce((sum, asset) => sum + asset.market_value, 0)
    ...
    <div className="flex min-h-screen w-full flex-col bg-muted/40">
      <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
        ...
        <Tabs defaultValue="stocks" className="space-y-4">
          <TabsList>
            <TabsTrigger value="stocks">股票</TabsTrigger>
            <TabsTrigger value="crypto">交易所加密貨幣</TabsTrigger>
            <TabsTrigger value="wallets">錢包資產</TabsTrigger>
            <TabsTrigger value="defi">DeFi 資產</TabsTrigger>
            <TabsTrigger value="cash">現金</TabsTrigger>
          </TabsList>
          <TabsContent value="stocks" className="space-y-4">
            <div className="flex justify-between">
              <h2 className="text-xl font-bold">股票資產</h2>
              <Button>新增股票</Button>
            </div>
            <StockAssetsList assets={stockAssets} />
          </TabsContent>
          <TabsContent value="crypto" className="space-y-4">
            <div className="flex justify-between">
              <h2 className="text-xl font-bold">交易所加密貨幣</h2>
              <Button>新增加密貨幣</Button>
            </div>
            <CryptoAssetsList assets={cryptoAssets} />
          </TabsContent>
          <TabsContent value="wallets" className="space-y-4">
            <div className="flex justify-between">
              <h2 className="text-xl font-bold">錢包資產</h2>
              <Button>新增錢包</Button>
            </div>
            <WalletAssetsList assets={walletAssets} />
          </TabsContent>
          <TabsContent value="defi" className="space-y-4">
            <div className="flex justify-between">
              <h2 className="text-xl font-bold">DeFi 資產</h2>
              <Button>新增 DeFi 資產</Button>
            </div>
            <DeFiAssetsList assets={defiAssets} />
          </TabsContent>
          <TabsContent value="cash" className="space-y-4">
            <div className="flex justify-between">
              <h2 className="text-xl font-bold">現金資產</h2>
              <Button>新增現金</Button>
            </div>
            <CashAssetsList assets={cashAssets} />
          </TabsContent>
        </Tabs>
      </main>
    </div>
